...
const ALLOWED_ORIGINS = [
  ...
  "http://localhost:5173",
  "http://localhost:3000",
  // Legacy Vercel URLs — keep so old deployments still work
  "https://frontend-rhid6kx7h-shreyasmegharaj2-2447s-projects.vercel.app",
];

app.use(cors({
  origin: (origin, callback) => {
    // Allow requests with no origin (curl, Postman, extensions)
    if (!origin) return callback(null, true);
    // Allow any *.vercel.app URL (covers all preview deployments)
